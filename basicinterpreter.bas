...
20 MODE7:PRINT:D=-1:DIM S% 255

REM debug flag
25 D%=0:D2%=0:D3%=0:D4%=0:D5%=0
...
90 PRINT:INPUT"HAPPY ADVENTURING... HIT ENTER TO START"K$:CLS:PRINT:RETURN
...
230 NEXTX:PROCd4("NV(0)="+STR$(NV(0))+" NV(1)="+STR$(NV(1))):F=NV(0)<1ORLEN(NT$(1))>0ANDNV(1)<1:RETURN
...
280 TP$=IA$(Z):IFRIGHT$(TP$,1)<>"/"THEN ELSE PROCd("280 FOR"):FOR W=LEN(TP$)-1TO1 STEP-1:IFMID$(TP$,W,1)="/"TP$=LEFT$(TP$,W-1):w=W:W=1:NEXTW:W=w:PROCd("280 NEXT 280a"):ELSE NEXT W:PROCd("280 NEXT 280b")
290 RETURN
...
360 F2=-1:F=-1:F3=0:IFNV(0)=1 AND NV(1)<7 THEN 610 ELSE PROCd("360 FOR"):FORX=0TOCL:V=INT(CA(X,0)/150):IF NV(0)=0 IFV<>0 x=X:X=CL:NEXTX:X=x:PROCd("360 NEXT 360"):RETURN
370 IF NV(0)<>V THEN NEXTX:PROCd("360 NEXT 370"):GOTO990 ELSE N=CA(X,0)-V*150
380 IF NV(0)<>0 ELSE F=0:IF RND(100)<=N THEN PROCd4("Random success"):G.400 ELSE PROCd4("Random fail"):NEXTX:PROCd("360 NEXT 380"):GOTO990
390 IF N<>NV(1) AND N<>0 THEN NEXTX:PROCd("360 NEXT 390"):GOTO990
395 PROCd4("NV(0)="+STR$(NV(0))+" V="+STR$(V)+" NV(1)="+STR$(NV(1))+" N="+STR$(N))

REM Process the conditions                        K=condition code, LL="number"
400 PROCd4("X="+STR$(X)):F2=-1:F=0:F3=-1:PROCd("400 FOR"):FORY=1TO5:W=CA(X,Y):LL=INT(W/20):K=W-LL*20:PROCd4("K="+STR$(K)+" LL="+STR$(LL)):F1=-1:ON K+1 GOTO550,430,450,470,490,500,510,520,530,540,410,420,440,460,480
...
550 F2=F2 AND F1:IF F2 THEN NEXTY:PROCd("400 NEXT 550a") ELSE y=Y:Y=5:NEXTY:Y=y:PROCd("400 NEXT 550b"):NEXTX:PROCd("360 NEXT 550"):GOTO990

REM Process the commands
560 IP=0:PROCd2("560 FOR"):FORY=1TO4:K=INT((Y-1)/2)+6:ON Y GOTO570,580,570,580
570 AC=INT(CA(X,K)/150):PROCd5("AC="+STR$(AC)):GOTO590
580 AC=CA(X,K)-INT(CA(X,K)/150)*150:PROCd5("AC="+STR$(AC))
...
890 PRINT"I'M CARRYING:":K$="NOTHING":PROCd3("890 FOR"):FORZ=0TOIL:IFIA(Z)<>-1THEN910 ELSEGOSUB280
900 PROCp(CHR$(32*-(POS>0))+TP$+"."):PROCj:K$=""
910 NEXT:PROCd3("890 NEXT 910"):PRINTK$:IFPOS PRINT':GOTO960 ELSEPRINT:GOTO960
...
REM Next command
960 NEXTY:PROCd2("560 NEXT 960")

REM Stop processing non-automatic actions
970 IF NV(0)<>0 THEN x=X:X=CL:NEXTX:X=x:PROCd("360 NEXT 970"):GOTO990

REM Next automatic action
980 NEXTX:PROCd("360 NEXT 980")
990 :
...
1110 K=0:PROCd3("1110 FOR"):FOR X=0TOIL:IF RIGHT$(IA$(X),1)<>"/"THEN1190 ELSE LL=LEN(IA$(X))-1:TP$=MID$(IA$(X),1,LL):FORY=LL TO2 STEP-1:IFMID$(TP$,Y,1)<>"/"THEN NEXTY:GOTO1190 ELSE y=Y:Y=2:NEXTY:Y=y
...
1170 PRINT"OK, ";:x=X:X=IL:NEXTX:X=x:PROCd3("1110 NEXT 1170")
1180 F=0:RETURN
1190 NEXTX:PROCd3("1110 NEXT 1190")
...
REM Debug FOR loops
2010 DEFPROCd(S$):IFD%P.S$:END. EL.END.
2020 DEFPROCd2(S$):IFD2%P.S$:END. EL.END.
2030 DEFPROCd3(S$):IFD3%P.S$:END. EL.END.

REM Debug conditions and actions
2040 DEFPROCd4(S$):IFD4%P.S$:END. EL.END.
2050 DEFPROCd5(S$):IFD5%P.S$:END. EL.END.

REM Word-wrap
2060 DEFPROCp($S%):LOCALA%,Z%,L%,C%,N%,T%:L%=40:N%=LEN$S%:A%=0:Z%=L%+1-POS:REPEATIFZ%>N%Z%=N%ELSEREPEATZ%=Z%-1:C%=S%?Z%:UNTILC%=32:IFZ%<A%Z%=A%+L%
...
2100 DEFFNb:LOCALA%,B%,C%:VDU11,8:A%=&87:B%=TRUE:FORI%=1TOL%:VDU9:C%=(USR(&FFF4)AND&FF00)DIV256:IFC%=32NEXT ELSEB%=FALSE:I%=L%:NEXT
2110 VDU10,13:=B%
